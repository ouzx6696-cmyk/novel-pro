...
| `draft.write` | Fast 或 Full 首稿正在形成 | `drafts/vol-N-ch-M.md` | 当前 writer 窗口完成或转入恢复 |
| `drafts.ready` | Fast 目标范围草稿已形成 | 目标范围内的 `drafts/` | 作者继续 Full 或另行处理草稿 |
| `review` | Full 正文正在冷读、返修或提交 | Reader 报告、task 候选、接受结论 | 目标范围全部复读接受并提交 |
...
| `fast.write` | `prompts.ready` → `draft.write` | Fast 单章草稿窗口 |
| `full.write` | `prompts.ready` → `draft.write`，完成后到 `review` | Full 首稿窗口 |
| `full.review` | `review` | 一幕顺序冷读 |
| `full.repair` | `review` | 按根因返修并复读 |
| `full.commit` | `review` → `volume.complete` 或下一目标阶段 | 接受正文提交 |
...
`novel-agent` 是唯一的控制面写入者：它独占 `.agent/status.yaml`、`.agent/order.yaml`、`.agent/tasks/<task-id>/` 的任务元数据、`.agent/run-log.yaml`，并执行 `full.commit` 向 `texts/` 的提交。角色返回报告或候选后，由顶层阅读并持久化；角色不能自行推进 cursor、order、run-log 或 `texts/`。
...
- 允许写入：`volumes/volume-N.md`；本卷任务明确分配的 `settings/` 与人物设定
- 返回顶层：卷纲、事实缺口、作者确认项
- 完成判定：卷纲、设定和文风交作者确认
- 下一跳：`outline.acts`
...
- 角色输入：context-pack（首任务为知识库原文）、当前幕纲、范围内章纲、`writing-style.md`、`genre-setting.md`、`world-setting.md`、相关人物设定、`writing-preferences.md`、`foreshadowing.md`、`timeline.md`（按章筛选所需）、承接入口
...
### fast.write
- 触发：`prompts.ready` 且作者选择 Fast；目标范围草稿尚未全部形成
...
### full.write
- 触发：`prompts.ready` 进入 Full；目标范围首稿尚未全部形成
- 加载模块：`skills/writing.md` + `skills/writer-construction.md`
- 创建角色：writer ×N（每章独立，范围 = 单章）
- 角色输入：顶层生成的单章 writer base、一个目标 Prompt
- 允许写入：首稿写 `drafts/vol-N-ch-M.md`；返修写当前 task candidate
...
### full.review
- 触发：`full.write` 完成（目标范围首稿形成）
- 加载模块：`skills/review-archive.md` + `skills/cold-read-discipline.md`
- 创建角色：reader ×1（范围 = 一幕顺序冷读）
- 角色输入：当前幕正文和候选；首读后才读契约、Prompt、知识
- 允许写入：不写项目产物
- 返回顶层：幕级 verdict（PASS/FIX/STOP）、已成立处、正文证据、根因、最小处理范围、保留项、建议处理角色（writer/prompt-crafter/planner/anti-ai）、接受候选和复读范围
...
- 加载模块：`skills/review-archive.md`（表达问题分流到 `skills/edit-boundary.md`）
- 创建角色：按根因分流 → 正文执行：新 writer；Prompt 设计：prompt-crafter；规划冲突：对应 planner；表达：anti-ai
- 角色输入：Reader 报告与受影响正文；根因分流决定对应角色的读取范围
- 允许写入：按分流写 draft candidate / 修复 Prompt / 重建规划 / 表达候选
- 返回顶层：分流结果与最小返修范围
- 完成判定：每个候选完成并进入复读
- 下一跳：Reader 重新顺序阅读受影响范围
- 恢复入口：保留原文与候选，按根因重新交对应角色
...
- **Fast**：见 `skills/writing.md`「Fast 流程 」
- **Full**：见 `skills/review-archive.md`「Full 创作流程」
...
`TASKS.md` 只是 Novel Desk 到顶层的作者请求适配器：顶层先汇总全部 `pending` 的来源路径、hash、范围和将采用的 operation，等待作者确认后才标记 `in_progress`。完成后回写 `completed`、`blocked` 或取消结果。它不写长期 cursor，不替代 order/task/run-log，也不改变任何角色所有权。
