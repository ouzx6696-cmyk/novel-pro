...
本模块是 anti-AI 与 completion-editor 共用的局部编辑权威边界。anti-AI 在编辑模式中先于 `edit.anti-ai` 阶段全量扫描同幕章节产出表达报告（不改正文），再于 `edit.repair` 阶段按 `edit.synthesize` 整体返修意见中归为中等/轻微的**表达类**问题产出局部候选；两种情形下都只处理有正文证据、边界清楚的问题，不自行扩大任务范围。completion-editor 仍只处理显式 `completion.revise` 中被分流为 `EDIT` 的局部问题。
