...
Reader 是编辑模式的内容裁判，按完整幕阅读。写作模式不加载本文件，也不运行 Reader 或 anti-AI。编辑模式的完整定义（工作目标/流程/调度）见 `SKILL.md` 的「编辑模式」节。
...
→ `edit.review`：Reader 按幕(目)冷读，出冷读报告
...
顶层按以下六步派发；每步的「读/写/判定」是精确接口：

| 步骤 | operation | 角色 | 读 | 写 | 判定 → 下一跳 |
|---|---|---|---|---|---|
| 1 | `edit.write` | writer ×N（每章独立） | 单章 base + 目标 Prompt | `drafts/vol-N-ch-M.md` | 首稿窗口完成 → 步骤 2 |
| 2 | `edit.review` | reader ×1（一幕） | 先只读幕正文（冷读），首读后才读 continuity contract 与诊断知识 | 冷读报告 | 报告给出 verdict 与复读范围 → 步骤 3 |
| 3 | `edit.anti-ai` | anti-ai ×1（同批） | 同批章节正文、`knowledge/anti-ai/index.md` | Anti-AI 报告 | 每章全量扫描列全 → 步骤 4 |
...
| 6 | `edit.commit` | novel-agent 自身 | 已复读接受候选、task 报告、目标路径 | `texts/vol-N-ch-M.md`、控制面文件 | 预检通过 → 幕间校准 → 下一幕 / `volume.complete` |

步骤 5 与步骤 6 之间强制经过 Reader 复读（按 `skills/cold-read-discipline.md` 复读范围判定清单重新顺序冷读）；未复读接受的候选不得提交。

各步执行要点：

- Reader 先只读当前幕正文，作为目标读者冷读，记录理解、期待、疑问、情绪和幕末感受；首读完成后才读取当前幕 continuity contract 和必要诊断知识，追查正文中真实出现的问题。
- 冷读报告先写成立处和真实阅读体验，再引用正文证据，给出根因、受影响章节、最小处理方式和复读范围；Reader 不直接改文或派发角色。
- anti-ai 对**同一批章节**做全量表达扫描，不依赖 Reader 点名，产出按幕(目)的 Anti-AI 报告（只列证据，不动文）。
...
`edit.anti-ai` 由顶层在 `edit.review` 之后派发 anti-ai，对**同一批章节**做表达扫描。它不再等待 Reader 点名，而是主动全量扫描 Reader 读过的全部章节，按幕(目)识别 AI 味、模板化表达、解释腔、机械重复、不自然对白等问题，产出 Anti-AI 报告。此阶段 anti-ai 只列证据与原句定位、标注严重倾向与是否越出局部编辑边界，不直接改正文，也不写 `texts/`；实际编辑在 `edit.repair` 阶段按 `edit.synthesize` 的整体返修意见执行。扫描可挂载 `knowledge/anti-ai/index.md`（通用与题材规则）。
