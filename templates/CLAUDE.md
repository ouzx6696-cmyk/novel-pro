...
`.agent/status.yaml` 保存长期 cursor 和 `migration` 状态；`.agent/order.yaml` 保存当前 operation、phase、卷幕、范围、批次、subtasks、attempt、反馈路径和任务状态；`.agent/tasks/<task-id>/` 保存报告、候选与恢复现场；`.agent/run-log.yaml` 记录重大失败、中断、重写和作者决策。只有 `novel-agent` 写这些控制面文件和 `texts/` 提交；`outline.acts` 是长期 cursor 阶段，`outline.act-map`、`outline.act` 是其中的临时 operation。
