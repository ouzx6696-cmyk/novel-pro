...
- 四步转化法（章纲 → Prompt 的过程）：锚定角色 → 锚定信息差 → 锚定情绪节奏 → 融合输出；与自检五步衔接（四步是写的过程、自检是写完的检查）
- 场景权重标注：每章 1 核心场景 + 至多 1 低权重转场（≤100 字）
- 案例元素：字段配笔记体正反例示范密度
- 自检结论：返回时按章附一行式自检表（六核对点：must_hold 落入/承接入口/人物发动机/场景过程/质感锚点/字段完整性）
...
- 质感核对（不写入 base）：构造时核对 Prompt「本章质感」为可执行具体材料，空泛时返回缺口；质感以 Prompt 为唯一指令源，不复制进 base
...
| `knowledge/plot/index.md` | 11 个剧情方法（冲突、钩子、幕结构、拆幕等） | vol/act/ch planner、prompt-crafter |
...
6. 文风执行 — 以 Prompt「本章质感」为唯一指令源 + 项目级声线硬规则（标点/禁用句式/章末纪律）
