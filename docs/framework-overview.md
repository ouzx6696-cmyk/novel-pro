...
- **通用写作底座**（跨题材、跨卷不变，回答"怎么写"）：webnovel（连载基线，含 fanqie-baseline）、plot（12）、scene（16）、character（4）、style（9 文风原型）。
- **类型风格知识**（题材专属，回答"这个题材的读者期待与禁忌"）：genre（26，25 个题材画像 + 索引，含 0.2.3-pro 新增的 `era-rebirth` 年代重生）、anti-ai（28，通用规则 + 25 个题材表达规则）。
