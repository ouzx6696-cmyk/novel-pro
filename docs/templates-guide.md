...
**用途**：项目声线的唯一权威源，**即项目的风格提示词**（prompt-crafter 每章调用、writer 执行），包含基准样章；可由作者随时触发风格蒸馏更新（上传样例文章 → 脱敏提取风格与文章结构 → 结合当前小说 → 生成可执行指示句）。

**何时填写**：
- volume-planner与作者共同形成
- 作者随时提出风格蒸馏时更新（`skills/planning.md`「风格蒸馏」）
- 作者确认后锁定

#### 核心结构

本文件分**两个部分**，对应两种消费方式（判断归属：写进章节叙述之前需要先决策的 → 第一部分；落笔时直接执行的 → 第二部分）：

```markdown
# 第一部分 · 本书写作基调与创作逻辑（逐章生效的创作决策依据）

## 1.1 写作基调
- 叙事者：{人称/限知/叙事者与主角的距离}
- 整体语言气质：{一句话概括，全章叙述示范的基调源头}
- 目标读者体感：{读者应有的情绪/节奏感受}
- 稳定与变化：必须稳定/可以变化/不要把变化误判为漂移

## 1.2 创作逻辑
- 情绪处理逻辑：{情绪命名还是外化？靠什么外化？何时允许短暂直白}
- 信息差管理逻辑：{谁在何时知道什么；信息差靠什么暴露，不对读者说什么}
- 节奏逻辑：{生活质地与事件推进如何交替；硬时钟事件如何锚定节奏}
- 群像组织逻辑：{配角如何写活；群像戏避免什么}
- 事实纪律：{时间/条款/数字等硬事实如何与时间线对齐}

## 1.3 文章结构
- 开篇切入：{从什么画面/事件直接进场}
- 场景推进：{进场→目标→阻力→反制→选择→余波}
- 转折节奏：{变化发生在哪一环}
- 收束方式：{如何停画面/留钩/换气}
...
- 对白：{口语程度/腔调/信息怎么露}
- 情绪：{命名还是外化/用哪些动作顿挫}
- 感官：{每章至少几处/落在哪些物上/禁止什么}
- 章末纪律：{停在哪种收束/禁止空升华和旁白预告}
...
**两部分的消费分工**：
- **第一部分** → prompt-crafter 写「本章故事」的叙述示范（视角、情绪外化、信息差暴露、节奏换气、结构组织）与「必守事实与边界」的创作逻辑约束（硬时钟、群像纪律、信息差边界）
- **第二部分** → prompt-crafter 逐场溶解为「本场声线」落点（句长倾向/对白配比/感官密度/留白/样句）；禁区核入本章边界
...
**低自由度原则**：风格提示词的各节一律写成**可执行、无歧义的指示句**（"动作段用短句，每句一个动作"），不用抽象形容词（"生动、克制、高级"）——抽象词留给执行者的自由度过大，是声线漂移与质量波动的根源。prompt-crafter 按指示句执行，跨章稳定由同一份风格提示词保证。
