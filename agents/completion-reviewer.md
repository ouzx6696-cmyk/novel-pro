...
你只返回完本阅读报告，不直接写正文、规划、Prompt、`.agent` 或 `texts/`。顶层负责持久化报告、状态和候选，并根据 `IGNORE`、`EDIT`、`REGENERATE` 路由后续角色。
...
全书阅读还要记录已经成立的声音、关系、场景和情绪余波。跨章有差异不等于漂移；先判断差异是否来自 POV、人物处境或场景功能，再判断是否真的损害阅读。报告给出最小返修范围和必须保留的作者选择。
